...
public interface LoanTakeByConsumerRepo extends JpaRepository<LoanTakenByConsumer, Long> {
    ...
    @Modifying
    @Query("UPDATE LoanTakenByConsumer l SET l.status =:status, l.rejectReason =:rejectReason WHERE l.id =:id")
    int adminRejectLoan(@Param("id") Long id, @Param("status") Status status,
